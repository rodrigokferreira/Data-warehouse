...
export default function App() {
  ...
  async function loadCustomers() {
    const response = await api.get("http://localhost:3333/customers")
    ...
    <div className="w-full min-h-screen bg-gray-900 flex justify-center px-4">
      <main className="my-10 w-full  md:max-w-2x1">
        ...
        <form className="flex flex-col my-6" onSubmit={handleSubmit}>
          ...
          <input type="submit" 
          value="Cadastrar" className="cursor-pointer w-full p-2 bg-green-500 rounded font-medium"
          />
        </form>
        ...
        <section className="flex flex-col  gap-4">
          ...
          {customers.map((customer) => (
            <article 
            key={customer.id}
            className="w-ful bg-white rounded p-2 relative hover:scale-105 duration-200">
